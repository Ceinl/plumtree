...
type SQLiteDriver struct {
	Extensions        []string
	BeforeConnectHook func(*SQLiteConn) error
...
func (d *SQLiteDriver) Open(dsn string) (driver.Conn, error) {
	...
	// BeforeConnectHook is intentionally earlier than all driver-owned PRAGMA
	// setup. SQLCipher must receive its key before a pragma such as synchronous
	// can inspect the encrypted pager. The hook is private to Plumtree's
	// replacement module and leaves the upstream ConnectHook ordering intact.
	if d.BeforeConnectHook != nil {
		if err := d.BeforeConnectHook(conn); err != nil {
			return fail(err)
		}
	}
