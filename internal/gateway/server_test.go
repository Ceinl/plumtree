...
func TestNewRejectsInvalidConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"missing backend", Config{}, "backend is required"},
		{
			...
			Config{Backend: backend, RunnerEndpoint: "unix:///run/plumtree.sock", RunnerWorker: "/bin/worker", RunnerToken: "tok"},
			"either runner endpoint or local runner worker",
		},
		{
			"endpoint requires a token",
			Config{Backend: backend, RunnerEndpoint: "unix:///run/plumtree.sock"},
			"runner token is required",
		},
		{
			"host commands require an allowlist",
			Config{Backend: backend, EnableHostCommands: true},
			"empty allowlist",
		},
	} {
