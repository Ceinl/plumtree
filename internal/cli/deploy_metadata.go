...
// DefaultServerURL and DefaultDevToken remain linker-injectable through the
// command entrypoint for private builds. Public release binaries leave both
// empty and use runtime configuration written by `pt --add-server` instead. A
// private build may inject them with:
//
//	go build -ldflags "\
