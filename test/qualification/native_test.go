...
func startServer(t *testing.T, binary, runnerWorker, config string, env []string) *liveServer {
	t.Helper()
	runner, _ := startReadyProcess(t, binary, []string{"serve", "--config", config, "--roles-control=false", "--roles-gateway=false", "--roles-runner=true", "--runtime-runner-worker", runnerWorker, "--product-version", "qualification"}, env, "runner     ")
