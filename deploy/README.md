...
- **Tokens.** Shared operator tokens are compared constant-time. Public `pt`
  binaries are generic and contain no deploy credential; authors configure
  servers and tokens at runtime with `pt --add-server URL ALIAS`. The command
  reads the token without exposing it in shell history and stores it in a
  user-only config file. Keep
